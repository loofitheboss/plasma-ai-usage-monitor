...
QString MistralProvider::model() const { return m_model; }
void MistralProvider::setModel(const QString &model)
{
    if (m_model != model) {
        m_model = model;
        Q_EMIT modelChanged();
...
void MistralProvider::fetchRateLimits()
{
    // Use a minimal chat completion to read rate limit headers
    // We use max_tokens=1 to minimize token usage
...
void MistralProvider::onCompletionReply(QNetworkReply *reply)
{
    reply->deleteLater();
    ...
    if (!doc.isNull()) {
        ...
        if (!usage.isEmpty()) {
            ...
            // These are per-request, but useful to confirm connectivity
